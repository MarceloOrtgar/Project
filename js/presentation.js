...
const inputPassword=document.querySelector(".signIn__form__containerPassword__inputPassword")

//Expresiones regulares
const regex_user=/^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;
const regexp_password = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,15}/;

//Datos erroneos
const wrongUser=document.querySelector(".signIn__form__alert__user")
const wrongPassword=document.querySelector(".signIn__form__alert__password")

//Mostrar/Ocultar Contraseña

const showHidepasswordContainer=document.querySelector(".signIn__form__containerPassword__containerShow")
const showHidepassword=document.querySelector(".signIn__form__containerPassword__containerShow__text")
...
window.addEventListener("click", (e) => {
 console.log(e.target)
  ...
  if(e.target.matches(".signIn__form__button")){
    if((regex_user.test(inputUser.value))&&(regexp_password.test(inputPassword.value))){
  ...
  if(e.target.matches(".signIn__form__containerPassword__containerShow__text")){
